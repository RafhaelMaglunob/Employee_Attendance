...
function Employees() {
	...
	const [renewEmployeeId, setRenewEmployeeId] = useState(null);
	...
	useEffect(() => {
		const handleOpenRenew = (e) => {
			setRenewEmployeeId(e.detail.employeeId);
			setIsRenewOpen(true);
		};
		window.addEventListener("openRenewContractModal", handleOpenRenew);
		return () => window.removeEventListener("openRenewContractModal", handleOpenRenew);
	}, []);
	...
	const filteredData = useMemo(() => {
		...
		switch (selectedSort) {
			case "(A-Z)": tempData = tempData.sort((a, b) => a.name.localeCompare(b.name)); break;
			case "(Z-A)": tempData = tempData.sort((a, b) => b.name.localeCompare(a.name)); break;
			case "Position": tempData = tempData.sort((a, b) => a.position.localeCompare(b.position)); break;
			case "Type": tempData = tempData.sort((a, b) => a.type.localeCompare(b.type)); break;
			case "Documents":
				tempData = tempData.sort((a, b) => {
					if (a.documents_complete === b.documents_complete) return 0;
					return a.documents_complete ? -1 : 1;
				});
				break;
			case "Status": tempData = tempData.sort((a, b) => a.status.localeCompare(b.status)); break;
			default: break;
		}
		return tempData;
	...
	const columns = [
		{ key: "number", title: "No.", render: (row) => row.number },
		{ key: "name", title: "Name" },
